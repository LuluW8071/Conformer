""" Script to evaluate conformer checkpoint using Greedy Decoding on LibriSpeech Validation Sets """
import os
import argparse
import torchaudio
import torch
...
def evaluate(model_path, token_path, dataset_path, split="test-clean"):
    ...
    for idx, (waveform, sample_rate, reference, *_ ) in tqdm(iterable, total=len(dataset), desc="Processing"):
        try:
            waveform = load_featurizer()(waveform)
            waveform = waveform.permute(0, 2, 1)

            emission = model(waveform)

            greedy_result = decoder(emission[0])
            prediction = " ".join(greedy_result)
